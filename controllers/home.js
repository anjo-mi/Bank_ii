...
export default {
  getHome: async (req, res) => {
    try {
      ...
      const userCategories = req.user?.id
        ? await Category.find({ userId: req.user.id })
        : null;
      const allCats = userCategories
        ? Array.from(
            new Set([
      ...
      const newSession = req.session.cookie.newSession && !req.user;
      const loggedIn = req.user ? true : false;
      req.session.cookie.newSession = false;

      res.render("index", { allCats, newSession, loggedIn });
  ...
  getSession: async (req, res) => {
    try {
      ...
      const session =
        await PracticeSession.findById(sessionId).populate("questions");
      const userQuestions = await Question.find({ userId: req.user.id });
      const parentIds = new Set(
        userQuestions
          .map((q) => (q.parentId ? q.parentId.toString() : null))
          .filter(Boolean),
      );
      ...
      const feedback = session.aiResponse
        ? session.aiResponse.questionResponse.map((res) => {
            const window = new JSDOM("").window;
            const pure = createDOMPurify(window);
            const purified = pure.sanitize(
              marked.parse(res.feedback, { breaks: true }),
            );
      ...
      const audioKeys = await Promise.all(
        session.audioKeys?.map(async (key) =>
          key ? await s3client.getAudio(key) : key,
        ),
      );
  ...
  updateUser: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "successfully updated user info" });
