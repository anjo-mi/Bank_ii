...
document.addEventListener("click", (e) => {
  ...
  while (elm) {
    if (
      elm.classList.contains("saved-feedback") ||
      elm.classList.contains("view-fb")
    )
      feedback = true;
    if (
      elm.classList.contains("view-saved") ||
      elm.classList.contains("saved-answer")
    )
      answer = true;
...
answerForm.addEventListener("submit", async (e) => {
  ...
  if ((e.target.id = "submit-answer")) {
    ...
    if (answer.trim().split(" ").length > 600 || answer.length > 6000) {
